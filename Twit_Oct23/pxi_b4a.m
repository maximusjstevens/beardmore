...
%demean ch1 waveforms along a transect
    ...
    Waveform_ch1=Waveform_ch1(:,1:n/2);
...
%demean ch0 waveforms along a transect
  ...
  Waveform_ch0=Waveform_ch0(:,1:n/2);
...
[b,a]=butter(4, [1 4]/50); %Max edit this line only.
...
figure; imagesc (Distance_total/1000, z_out, filtdata, [-.5/100 .5/100]); grid on;  
colormap (bone(256)); 
axis([0 max(Distance_total/1000) 0 max(z_out)]);
xlabel('Distance (km)'); ylabel('Ice thickness(m)');

%figure; imagesc (Distance_total/1000, z_out, filtdata2, [-.5/100 .5/100]); grid on;  
%colormap (bone(256)); 
%axis([0 max(Distance_total/1000) 0 max(z_out)]);
%xlabel('Distance (km)'); ylabel('Ice thickness(m)');
